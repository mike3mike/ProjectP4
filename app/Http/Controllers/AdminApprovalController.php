...
class AdminApprovalController extends Controller
{
    ...
    public function declineAssignment(Task $task)
    {
        $user = User::find($task->client_id);
        $user->notify(new AssignmentDeclined($task));
        $task->delete(); // Keur de opdracht goed en sla het op
        return back()->with('status', 'Opdracht afgewezen.'); // Keer terug naar de vorige pagina met een succesbericht
    }
}
